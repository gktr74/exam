...
foreach($arResult as $arItem):?>
	...
	<?if ($arItem["IS_PARENT"]):?>
		...
		<?if ($arItem["DEPTH_LEVEL"] == 1):?>
			<li class="close"><a href="<?=$arItem["LINK"]?>"><span><?=$arItem["TEXT"]?></span></a>
				<ul>
		<?else:?>
			<li class="open current"><a href="<?=$arItem["LINK"]?>" class="parent<?if ($arItem["SELECTED"]):?> item-selected<?endif?>"><?=$arItem["TEXT"]?></a>
				<ul>
		<?endif?>
	...
	<?else:?>
		...
		<?if ($arItem["PERMISSION"] > "D"):?>
			...
			<?else:?>
				<li class="close"><a href="<?=$arItem["LINK"]?>" <?if ($arItem["SELECTED"]):?><li class="close"> class="item-selected"<?endif?>><?=$arItem["TEXT"]?></a></li>
			<?endif?>
